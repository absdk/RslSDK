...
if ($foundDbContextType) {
    ...
    $modelName = Get-SingularizedWord $foundModelType.Name
	...
	if (!$isDbContext) {
		...
		} -SuccessMessage "Added '$modelName' to database context '$($foundDbContextType.FullName)'" -TemplateFolders $TemplateFolders -Project $Project -CodeLanguage $CodeLanguage
	}
}

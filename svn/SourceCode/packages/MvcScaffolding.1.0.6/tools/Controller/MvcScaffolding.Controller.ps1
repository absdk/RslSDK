...
if($ParentModelName)
{
Write-Host "Parent Model Name : "$ParentModelName
}

if($BaseControllerName)
{
Write-Host "Base Controller Name : "$BaseControllerName
}

$ControllerName=[string](Get-SingularizedWord $ControllerName)
...
if (!$ModelType) {
	if ($ControllerName.EndsWith("Controller", [StringComparison]::OrdinalIgnoreCase)) {
		...
		if ($foundModelType) {
			$ControllerName = [string](Get-SingularizedWord $foundModelType.Name) + "Controller"
		}
	}
...
if (!$NoChildItems) {
	if ($Repository) {
		Write-Host "Scaffolding Repository Started..."
		Scaffold Repository -ModelType $foundModelType.FullName -DbContextType $DbContextType -Area $Area -Project $Project -CodeLanguage $CodeLanguage -Force:$overwriteFilesExceptController -BlockUi -ParentModelName $ParentModelName
		Write-Host "Scaffolding Repository Completed..."
	} else {
...
$repositoriesNamespace = [T4Scaffolding.Namespaces]::Normalize($Project + ".BusinessLogic.Repositories")
...
if($ParentModelName)
{
$templateName = if($Repository) { "ControllerWithRepositoryChild" } else { "ControllerWithContext" }
}
else
{
$templateName = if($Repository) { "ControllerWithRepository" } else { "ControllerWithContext" }
}
Write-Host "Scaffolding Controller Started..."
...
Write-Host "Scaffolding Controller Completed..."
if (!$NoChildItems) {
	...
	if ($ViewScaffolder) {
		Write-Host "Scaffolding Views Started..."
		Scaffold Views -ViewScaffolder $ViewScaffolder -Controller $controllerNameWithoutSuffix -ModelType $foundModelType.FullName -Area $Area -Layout $Layout -SectionNames $SectionNames -PrimarySectionName $PrimarySectionName -ReferenceScriptLibraries:$ReferenceScriptLibraries -Project $Project -CodeLanguage $CodeLanguage -Force:$overwriteFilesExceptController -ParentModelName $ParentModelName
		Write-Host "Scaffolding Views Completed..."
	}
}
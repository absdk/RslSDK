...
namespace SoftwareGrid.BusinessLogic.Repositories.Context
{
    public class RepositoryContextInitializer : DropCreateDatabaseIfModelChanges<RepositoryContext>, IRepositoryContextInitializer
    {
        ...
        protected override void Seed(RepositoryContext context)
        {
            ...
            foreach (VendorStatusCategory vendorStatusCategory in VendorStatusCategorySeed.VendorStatusCategories)
            {
                context.VendorStatusCategory.Add(vendorStatusCategory);
            }

            foreach (VendorAddressCategory vendorAddressCategory in VendorAddressCategorySeed.VendorAddressCategories)
            {
                context.VendorAddressCategory.Add(vendorAddressCategory);
            }

            foreach (VendorDocumentCategory vendorDocumentCategory in VendorDocumentCategorySeed.VendorDocumentCategories)
            {
                context.VendorDocumentCategory.Add(vendorDocumentCategory);
            }

            foreach (VendorDocumentStatusCategory vendorDocumentStatusCategory in VendorDocumentStatusCategorySeed.VendorDocumentStatusCategories)
            {
                context.VendorDocumentStatusCategory.Add(vendorDocumentStatusCategory);
            }

            //foreach (VendorInsuranceCategory vendorInsuranceCategory in VendorInsuranceCategorySeed.VendorInsuranceCategories)
            //{
            //    context.VendorInsuranceCategory.Add(vendorInsuranceCategory);
            //}

            foreach (VendorLegalCompanyStructureCategory vendorLegalCompanyStructureCategory in VendorLegalCompanyStructureCategorySeed.VendorLegalCompanyStructureCategories)
            {
                context.VendorLegalCompanyStructureCategory.Add(vendorLegalCompanyStructureCategory);
            }

            foreach (VendorNoteCategory vendorNoteCategory in VendorNoteCategorySeed.VendorNoteCategories)
            {
                context.VendorNoteCategory.Add(vendorNoteCategory);
            }

            foreach (VendorSupplierDiversityCertificationCategory vendorSupplierDiversityCertificationCategory in VendorSupplierDiversityCertificationCategorySeed.VendorSupplierDiversityCertificationCategories)
            {
                context.VendorSupplierDiversityCertificationCategory.Add(vendorSupplierDiversityCertificationCategory);
            }


            foreach (VendorServiceCategory vendorServiceCategory in VendorServiceCategorySeed.VendorServiceCategories)
            {
            ...
            foreach (OrganizationAddressCategory organizationAddressCategory in OrganizationAddressCategorySeed.OrganizationAddressCategories)
            {
                context.OrganizationAddressCategory.Add(organizationAddressCategory);
            }

            foreach (OrganizationCategory organizationCategory in OrganizationCategorySeed.OrganizationCategories)
            {
